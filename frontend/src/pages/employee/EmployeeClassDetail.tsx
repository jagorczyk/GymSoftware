              ...
              {reservations.map((r) => (
                <tr key={r.id} className="hover:bg-slate-50 transition-colors">
                  <td className="px-6 py-4 font-bold text-slate-900">
                    <Link to={`/employee/guests/${r.guestId}`} className="hover:text-primary-600 hover:underline">
                      {r.guestFirstName} {r.guestLastName}
                    </Link>
                    <div className="text-xs text-slate-500 font-normal">{r.guestEmail}</div>
                  </td>
                    ...
                    {r.status !== "CANCELLED" && (
                      <>
                        <button
                          ...
                          className={`p-2 rounded-lg transition-colors ${r.status === "ATTENDED" ? "bg-emerald-100 text-emerald-600 opacity-50 cursor-not-allowed" : "text-emerald-600 hover:bg-emerald-50"}`}
                          title="Obecny"
                        >
                        ...
                        <button
                          ...
                          className={`p-2 rounded-lg transition-colors ${r.status === "NO_SHOW" ? "bg-rose-100 text-rose-600 opacity-50 cursor-not-allowed" : "text-rose-600 hover:bg-rose-50"}`}
                          title="Nieobecny"
                        >
