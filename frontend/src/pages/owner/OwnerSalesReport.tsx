    ...
    <div className="space-y-6">
      <FormSection title="Zakres raportu">
        <div className="flex flex-wrap items-end gap-4">
          ...
          <button type="button" onClick={handleDownloadCsv} disabled={loading} className="bg-slate-100 hover:bg-slate-200 text-slate-800 border border-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 dark:border-slate-700 dark:text-slate-200 py-2 px-4 rounded-xl font-bold transition-all disabled:opacity-50 text-sm">
            Pobierz CSV
          </button>
        </div>
      </FormSection>
        ...
        <>
          <div className={`grid grid-cols-1 sm:grid-cols-2 ${report.productRevenue != null ? 'md:grid-cols-3' : ''} gap-4`}>
            ...
            {report.productRevenue != null && (
              <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-5 transition-colors duration-200">
                <p className="text-sm text-slate-500 dark:text-slate-400">Przychód z POS</p>
